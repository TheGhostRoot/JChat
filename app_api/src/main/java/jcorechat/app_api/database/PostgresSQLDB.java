...
CREATE TABLE profiles (
    ...
    FOREIGN KEY (id) REFERENCES accounts(id)
        ...
        msg: 5msg{pinned}
        index 0 = amount of chars for Datetime
        for example index 0 is the number 5 then the next 5 chars are the date time
        after the 5 chars is the message
        at the end it says if it is pinned or not
    ...
    PostgresSQLDB() {
        try {
            ...
            deleteTable("posts");
            deleteTable("chats");
            deleteTable("captchas");
            deleteTable("conversations");
            deleteTable("accounts");

            createTable("accounts", accounts_table);
            createTable("conversations", conversations_table);
            createTable("captchas", captchas_table);
            createTable("profiles", profiles_table);
            createTable("posts", posts_table);
            createTable("chats", chats_table);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public Long createUser(String name, String email, String password, String encryption_key, String sign_key) {
        if (null == postgressql_connection) {
            return null;
        }
        ...
        if (!addData(DatabaseManager.table_accounts,
        ...
        Map<String, List<Object>> account_data = getData(DatabaseManager.table_accounts, "id",
        ...
        if (!addData(DatabaseManager.table_profiles, "id, pfp, banner, pets, coins, badges, animations",
    ...
    public boolean changeUserEmail(long id, String new_email) {
        ...
        return editData(DatabaseManager.table_accounts, "email = ?", account_set,
                "id = ?", account_where);
    }
    public boolean changeUserPassword(long id, String new_password) {
        ...
        return editData(DatabaseManager.table_accounts, "password = ?", account_set,
                "id = ?", account_where);
    }
    public boolean changeUserEncryptionKey(long id, String new_encryptino_key) {
        ...
        return editData(DatabaseManager.table_accounts, "encryption_key = ?", account_set,
                "id = ?", account_where);
    }
    public boolean changeUserSignKey(long id, String new_sign_key) {
        ...
        return editData(DatabaseManager.table_accounts, "sign_key = ?", account_set,
                "id = ?", account_where);
    }
    public boolean changeUserSessionID(long id, Long session_id) {
        ...
        return editData(DatabaseManager.table_accounts, "session_id = ?", account_set,
                "id = ?", account_where);
    }
    public boolean changeUserSessionExpire(long id, short session_expire)  {
        ...
        Map<String, List<Object>> sess_data = getData(DatabaseManager.table_accounts, "last_edit_time",
        ...
        return editData(DatabaseManager.table_accounts, "session_expire = ?", account_set,
                "id = ?", account_where);
    }
    ...
    public boolean changeUserSessionSuspended(long id, String stats) {
        ...
        return editData(DatabaseManager.table_accounts, "session_suspended = ?", set_data,
                "id = ?", condition_data);
    }
    private boolean isUserSessionSuspended(long id) {
        ...
        Map<String, List<Object>> data = getData(DatabaseManager.table_accounts, "session_suspended",
    ...
    public boolean addUserFriend(long id, long friend_id) {
        ...
        return editData(DatabaseManager.table_accounts, "friends = friends || ?", account_friends,
                "id = ?", account_where);
    }
    public boolean removeUserFriend(long id, long friend_id) {
        ...
        return editData(DatabaseManager.table_accounts, "friends = REPLACE(friends, ?, '')", account_friends,
                "id = ?", account_where);
    }
    public boolean addUserGroup(long id, long group_id) {
        ...
        return editData(DatabaseManager.table_accounts, "groups = groups || ?", account_friends,
                "id = ?", account_where);
    }
    public boolean removeUserGroup(long id, long group_id) {
        ...
        return editData(DatabaseManager.table_accounts, "groups = REPLACE(groups, ?, '')", account_friends,
                "id = ?", account_where);
    }
    ...
    public Long createConvID(long party_id, long party_id2) {
        ...
        Map<String, List<Object>> conv_data = getData(DatabaseManager.table_conversations, "conv_id", "",
                null, null, "", 0);
        ...
        set_data.add(id);
        return addData(DatabaseManager.table_conversations, "party_id, party_id2, conv_id", "?, ?, ?", set_data) ?
                id : null;
    }
    public boolean addMessage(long conv_id, long sender_id, String message) {
        ...
        Map<String, List<Object>> current_chat_data = getData(DatabaseManager.table_chats,
                "msg, msg_id", "conv_id = ?",
        ...
        String format  = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm").toString();
        chat_data.add(""+format.length()+format+"|"+message);
        ...
        if ((current_chat_data.get("msg").isEmpty()) ||
                (!editData(DatabaseManager.table_chats, "msg = msg || ?" , set_data,
                        "conv_id = ?", edit_condition_data))) {
            return addData(DatabaseManager.table_chats, "conv_id, msg, sent_by, msg_id",
                    "?, ?, ?, ?, ?", chat_data);
    ...
    public Map<String, List<Object>> getMessages(long sender_id, long conv_id, int amount) {
        ...
        return getData(DatabaseManager.table_chats, "*", "conv_id = ?", where_values, null,
                "sent_at DESC", amount);
    }
    public boolean deleteMessage(long sender_id, long conv_id, long message_id) {
        ...
        return deleteData(DatabaseManager.table_chats, "conv_id = ? AND msg_id = ? AND sent_by = ?", condition_data);
    }
    ...
    public Long startCaptcha(String answer) {
        Map<String, List<Object>> data = getData(DatabaseManager.table_captchas, "id", "",
                null, null, "", 0);
        ...
        if (!addData(DatabaseManager.table_captchas, "id, answer, time, last_edit_time, failed", "?, ?, 10, ?, 0", values))
        { return null; }
    ...
    public boolean verifyCaptcha(long id, String given_answer) {
        ...
        Map<String, List<Object>> captcha_data = getData(DatabaseManager.table_captchas, "answer, time, failed",
                "id = ?", condition_data, null, "", 0);
        ...
        if (captcha_data.get("answer").contains(given_answer)) {
            // solved!
            return deleteData(DatabaseManager.table_captchas, "id = ?", condition_data);

        } else if ((3 <= (short) captcha_data.get("failed").get(0)) ||
                (0 >= (short) captcha_data.get("time").get(0))) {
            // extended fails or time

            deleteData(DatabaseManager.table_captchas, "id = ?", condition_data);
            ...
            if (!(captcha_data.get("time").isEmpty() || captcha_data.get("failed").isEmpty())) {
                editData(DatabaseManager.table_captchas, "failed = failed + 1", null, "id = ?",
                        condition_data);
            }
            return false;
        }
    ...
    public boolean updateCaptchaTime(long id) {
        ...
        Map<String, List<Object>> sess_data = getData(DatabaseManager.table_captchas, "last_edit_time",
                "id = ?", condition_data, null, "", 0);
        ...
        return editData(DatabaseManager.table_captchas, "time = time - 1", null, "id = ?", condition_data);
    }
    ...
    public boolean createPost(long sender_id, String msg, String tags, String background) {
        ...
        return addData(DatabaseManager.table_posts, "sender_id, msg, tags, send_at, background", "?, ?, ?, ?, ?", data);
    }
    public boolean deletePost(long sender_id, long post_id) {
        ...
        return deleteData(DatabaseManager.table_posts, "id = ? AND sender_id = ?", condition_data);
    }
    public boolean editPost(long sender_id, long post_id, String edited_tags,
        ...
        return editData(DatabaseManager.table_posts, "msg = ?, tags = ?, background = ?", set_data,
    ...
    public boolean updateProfilePfp(long id, String given_pfp) {
        ...
        return editData(DatabaseManager.table_profiles, "pfp = ?", profile_data, "id = ?", condition_data);
    }
    public boolean updateProfileBanner(long id, String given_banner) {
        ...
        return editData(DatabaseManager.table_profiles, "banner = ?", profile_data, "id = ?", condition_data);
    }
    public boolean updateProfilePets(long id, String given_pets) {
        ...
        return editData(DatabaseManager.table_profiles, "pets = ?", profile_data, "id = ?", condition_data);
    }
    public boolean updateProfileCoins(long id, int given_coins) {
        ...
        return editData(DatabaseManager.table_profiles, "coins = ?", profile_data, "id = ?", condition_data);
    }
    public boolean updateProfileBadges(long id, String given_badges) {
        ...
        return editData(DatabaseManager.table_profiles, "badges = ?", profile_data, "id = ?", condition_data);
    }
    public boolean updateProfileAnimations(long id, String given_animations) {
        ...
        return editData(DatabaseManager.table_profiles, "animations = ?", profile_data, "id = ?", condition_data);
    }
    ...
    public boolean deleteUser(long id) {
        ...
        if (!deleteData(DatabaseManager.table_accounts, "id = ?", data)) { return false; }
        if (!deleteData(DatabaseManager.table_profiles, "id = ?", data)) { return false; }

        data.clear();
        data.add(id);
        if (!deleteData(DatabaseManager.table_posts, "sender_id = ?", data)) { return false; }

        data.clear();
        data.add(id);
        data.add(id);
        return deleteData(DatabaseManager.table_chats, "id = ? OR id2 = ?", data);
    ...
    private boolean deleteData(String table, String condition, List<Object> conditionData) {
        ...
        try {
            ((PreparedStatement) setData((short) 1, conditionData,
                    postgressql_connection.prepareStatement(stringBuilder.toString())).get(1)).executeUpdate();
            return true;
        } catch (Exception e) {
            return false;
        }
    ...
    private boolean addData(String table, String fields, String values, List<Object> data) {
        ...
        if (null == postgressql_connection) { return false; }

        try {

            ( (PreparedStatement) setData((short) 1, data, postgressql_connection.prepareStatement(new StringBuilder("INSERT INTO ")
    ...
    private boolean editData(String table, String set_expression, List<Object> set_data, String condition,
        ...
        try {
            List<Object> data = setData((short) 1, set_data, postgressql_connection.
    ...
    private Map<String, List<Object>> getData(String table, String data_to_get, String condition,
        ...
        try {
            ...
            PreparedStatement preparedStatement = postgressql_connection.prepareStatement(select_query.append(";")
                    .toString());
    ...
    public boolean createTable(String table, List<String> colums) {
        ...
        try {
            ...
            readOutput(postgressql_connection.prepareStatement(stringBuilder.append(" );").toString()).executeQuery());
            return true;
        } catch (Exception  e) {
            return false;
        }
    ...
    public boolean deleteTable(String table) {
        ...
        try {
            ...
            postgressql_connection.prepareStatement(stringBuilder.toString()).executeQuery();
            return true;
        } catch (Exception  e) {
            return false;
        }
